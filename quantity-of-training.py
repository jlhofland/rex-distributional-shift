...
def runSampleComplexityExp():
    ...
    # Environments
    environments.append(["0.2","2.","5.0"])
    environments.append(["0.2","2.","0.5","5.0"])
    environments.append(["0.2","2.","0.5","0.8","5.0"])
    environments.append(["0.2","2.","0.5","0.8","1.1","5.0"])
    environments.append(["0.2","2.","0.5","0.8","1.1","1.4","5.0"])
    environments.append(["0.2","2.","0.5","0.8","1.1","1.4","1.7","5.0"])

    # Numbers of training environments
    env_labels = [2,3,4,5,6,7]
    ...
    for o in range(len(options)):
        ...
        for s in shifts:
            ...
            for e in range(len(environments)):
                print("-- Environment: " + str(environments[e]))

                # Run for current Shift, Samples and Environment
                settings = getSettings(env_list=",".join(environments[e]), n_reps=5, n_iterations=100, shift=s, methods=",".join(methods), setup_ones=options[o][0], setup_hetero=options[o][1], setup_scramble=options[o][2], setup_confounder_on_x=options[o][3])
            ...
            for t in res.keys():
                # Path to file
                path = "./results/quantity-of-training/" + config_string + "/" + t + "/"
                ...
                table = tbl.tabulate(rows, headers=["Method", "Number of training domains", "Model estimation error", "Standard error"])
