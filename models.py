...
# Edited to allow for multiple test domains
def compute_softmax_negentropy(losses, iter):
    ps = F.softmax(torch.stack(losses), dim=0)
    res = 0
    for v in ps:
        res += v * torch.log(v)
    return res
...
class REXv21(object):
    def __init__(self, environments, args):
        ...
        for reg in [1, 10, 100, 1000, 10000, 100000]:
            ...
            if args["verbose"]:
                print("REXv21 (reg={:.3f}) has {:.3f} validation error.".format(reg, err))
        ...
        # Added to get rid of error best_phi assignment before defining if REx does not converge
        if(torch.is_tensor(best_phi)):
            self.phi = best_phi
    ...
    def train(self, environments, args, reg=0, use_cuda=False):
        ...
        for iteration in range(args["n_iterations"]):
            ...
            # Edited to allow for multiple environments
            for x_e, y_e in environments:
                error_e = MSE(x_e @ self.phi, y_e)
            ...
            loss = 0.0
            loss += beta * compute_softmax_negentropy(errors, iteration)
            loss += sum(errors)
