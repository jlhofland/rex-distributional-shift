...
def runSampleComplexityExp():
    ...
    # Overwrite for homoscedastic with 1 to 1 correlation of invariant features
    options = [[1,0,0,0]]
    ...
    for o in range(len(options)):
        ...
        for s in shifts:
            ...
            for n in n_samples:
                print("- # SAMPLES: " + str(n))
                settings = getSettings(n_samples=n, n_reps=5, n_iterations=100, shift=s, methods=",".join(methods), setup_ones=options[o][0], setup_hetero=options[o][1], setup_scramble=options[o][2], setup_confounder_on_x=options[o][3])
            ...
            for t in res.keys():
                path = "./results/sample-complexity/" + config_string + "/" + t  + "/"
                # Create plot
                for m in methods:
                    plt.plot(n_samples, res[t][m][s][0], label=m, marker=markers[m])
                ...
                for i in range(len(n_samples)):
                    for m in methods:
                        rows.append([m, n_samples[i], res[t][m][s][0][i], res[t][m][s][1][i]])
                table = tbl.tabulate(rows, headers=["Method", "Number of samples", "Model estimation error", "Standard error"])
